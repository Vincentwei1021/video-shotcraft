...
规则：强节奏 BGM 片"切点落拍"不等于"逐拍打画面"——作用于整画面/相机层的冲击（整画面 scale 泵/震屏/闪帧/负片帧）视同大 slam 管理：全片 ≤3 处、钉 music-beat-sync §2b 的最强 hit 清单、相邻两处间隔 ≥16 拍；其余拍点动效只作用主角元素层；节拍泵类连打单次 ≤4 拍、全片 ≤1 次。kick 命中表是候选池不是触发器。
...
### S5. 音画对齐补两项延迟：`<Audio>` 起播滞后逐片实测统一扣，音源峰值滞后逐文件量
规则：Remotion `<Audio>` 实际发声晚于 `Sequence.from`，且滞后量逐片不同——必须对本片实测（渲出成片抽音轨，拿 2–3 个尖锐 SFX 原文件做归一化交叉相关，残差一致才算量准），补偿在渲染层统一扣除，不逐条改帧号；音源自身的峰值滞后（峰值不在文件头）逐文件量，钉帧 = 目标帧 − 峰值滞后。两项都要补，方法见 sound-design §4.6。
判例：promo-ink 实测起播滞后 4f（钤印/落款首字/S5 行笔三处独立动作残差一致），AiflPromo 实测仅 +1.3f（click-camera/impact-cine/pop 三探针交叉相关）——不是普适常数。不补时两项叠加最重一击晚 0.27s，明显读作音画不同步；卡点片里差 2–3 帧就足以让合理的卡点动效被读作无端抖动（2026-08-22 用户反馈的成因之一）。
自检：本片的起播滞后量实测过吗（记录在哪）？补偿是渲染层统一扣的还是散落在各条帧号里？带明显攻击峰的音源，峰值滞后量过了吗？
