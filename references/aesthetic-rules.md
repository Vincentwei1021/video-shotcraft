...
**验法（可自动化，别只靠眼看）：** "真静止"要求静止期一切属性锁死——包括辉光、阴影、高光残留、以及缓动尾巴上肉眼不可见的余量。这等价于**逐像素相同**，所以直接渲静止段的首帧和末帧比哈希即可：

```bash
npx remotion still src/index.ts <Comp> a.png --frame=<静止起点>
npx remotion still src/index.ts <Comp> b.png --frame=<镜头末帧>
shasum -a256 a.png b.png   # 两行哈希不同 = 还有东西在动
```

比看静帧严得多。实战里它抓到过一处眼睛看不出来的违例：一个 `1 + 0.08·exp(-t/3)` 的顿推泵，窗口 14f，但静止起点被按错位窗（12f）算了——t=12 时 scale 还是 **1.0015**，0.15% 的缩放不可见，却让"27f 真静止"变成了假话。**取所有并行窗口里最晚归零的那个**当静止起点。
