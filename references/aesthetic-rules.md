...
- 库里 `sfx/ui/` 与 `sfx/glass/` 不是禁区：`ui/` 装的是真实开关/轻点质感（`switch-light` `switch-click-quick` 等），服务 theme-switch-moves、segmented-thumb-hero、command-palette-summon 这类卡；`glass/` 是碎裂材质音。按上面的问句逐个试听取舍即可。
...
操作要点（库里的两份特殊名单，见 sound-design 4.1）：**>5s 的长样本（21 个）必须显式给 `durationInFrames`**，漏给会拖过动作结束；带长尾混响的 impact 例外，让尾音自然衰减。**峰值 <-12dB 的轻音素材（7 个）** 给常规 0.2–0.6 等于没声，要抬到 0.8+。
