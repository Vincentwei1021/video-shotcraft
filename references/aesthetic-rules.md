...
### Q1. 复刻既有页面必须来自浏览器真实渲染；HTML 优先，截图只作 QA 与最后兜底
规则：凡是表现产品既有页面的镜头，先按 `html-material-gate.md` 选择 B 整页 HTML、A 冻结子树或 B+A；同步截图只作 baseline，不进入成片。只有 Canvas/WebGL/video/跨域 iframe、保真/确定性门控失败等有证据的情况才允许 raster fallback，不能因为截图更快就降级。非复刻场景允许手搓 UI，但质量（出版级质感，对照 Q10）与表达明确性是硬门槛。公开演示数据只有在产品简报明确确认后可保留；客户、个人、内部、密钥、实时或其他敏感内容必须先虚构、脱敏或冻结。
判例：早期用户要求“真实页面 + 动画”后，低质量手搓复刻被真实页面素材取代；2026-07-13 裁决允许非复刻 UI 在质量达标时手搓。2026-08-24 新裁决进一步要求：素材按分镜门控选择 A/B，“尽量尽量别用截图的形式放入视频”。因此“真实”指浏览器真实渲染，不再等同于截图。
自检：每块 UI 素材是否有 A/B/B+A 门控记录？baseline PNG 是否被误当成视频素材？若使用 raster，是否有具体失败证据？源数据是否符合口径且无敏感信息？

### Q2. HTML 素材保持浏览器重绘；只有 raster fallback 才走 2–4 倍高分纹理链路
规则：A/B HTML 素材不得预先栅格化，推近时保持 DOM、字体、SVG 与 CSS 图形由浏览器按目标倍率重绘；文字模糊先查 iframe/fragment 的合成与缓存层。只有门控确认 raster fallback 时，UI 纹理才按显示尺寸的 2–4 倍栅格化并向下采样（放大走布局级 CSS zoom 而非 transform scale）。别用 DoF 掩盖素材链路问题。
...
自检：放大/透视镜头逐帧看文字边缘：A/B 是否仍由浏览器清晰重绘？若为 raster fallback，纹理源分辨率是否 ≥ 显示尺寸 2 倍？

技法展开（以下仅适用于 raster fallback；HTML 实现读 `html-material-gate.md`，参考组件 `assets/lib/HtmlPageCam.tsx` / `FrozenHtmlFragment.tsx`）：
