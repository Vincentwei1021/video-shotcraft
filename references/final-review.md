...
- `shot-material-plan.md`、各状态 State Bundle 的离线 QA 结果与 raster fallback 证据；
...
- D3：表现真实产品页面时是否按门控使用 A/B/B+A 浏览器真实 HTML，而非低质量
  手搓复刻；是否把仅供 QA 的 baseline PNG 误放进成片；
- D4：HTML 状态、字体、图片和动态数据是否完整加载；是否通过零外网请求、保真和
  确定性验收。使用 raster fallback 时是否有明确触发条件与验证证据；
